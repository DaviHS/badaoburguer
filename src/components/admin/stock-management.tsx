...
export function StockManagement() {
  const { data: productsData } = api.product.list.useQuery<Product[]>()
  ...
  const handleStockChange = (productId: number, value: number) => {
    setProducts((prev) =>
      prev.map((p) => (p.productId === productId ? { ...p, stock: Math.max(0, value) } : p)),
    )

    if (timeoutRefs.current[productId]) clearTimeout(timeoutRefs.current[productId])

    timeoutRefs.current[productId] = setTimeout(() => {
      updateStockMutation.mutate({ productId, stock: Math.max(0, value) })
    }, 500)
  }
            ...
            {filteredProducts.map((product) => (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  >
                    {product.stock === 0 ? "Sem estoque" : product.stock <= 5 ? "Baixo" : "Normal"}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-center gap-2 mt-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleStockChange(product.productId, product.stock - 1)}
                  ...
                  <Input
                    ...
                    value={product.stock}
                    min={0}
                    onChange={(e) => handleStockChange(product.productId, Number(e.target.value) || 0)}
